...
```json
{
  "mcpServers": {
    "biliscope-mcp": {
      "command": "npx",
      "args": ["-y", "biliscope-mcp@2.1.7", "stdio"],
      "env": {
...
- `command` 必须是 `npx`。
- `args` 建议写死 `biliscope-mcp@2.1.7`，避免 ModelScope 缓存旧版本。
...
```json
{
  ...
  "message": "CookieCloud 配置缺失：COOKIECLOUD_ENDPOINT, COOKIECLOUD_UUID, COOKIECLOUD_PASSWORD。"
}
```
...
- 用户 KEY / UUID：填入部署 JSON 的 `COOKIECLOUD_UUID`。
- 端对端加密密码：填入部署 JSON 的 `COOKIECLOUD_PASSWORD`。
...
```bash
npx -y biliscope-mcp@2.1.7 http
```
...
```bash
npx -y biliscope-mcp@2.1.7 stdio
```
